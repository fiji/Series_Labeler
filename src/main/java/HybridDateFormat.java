...
date constructs based on SimpleDateFormat.java
...
public class HybridDateFormat extends DateFormat {
	/**
	 * A pair class used by SimpleDateFormat as a compiled representation of a
	 * format string.
	 */
			...
			if (field == -1) {
				...
				if (Character.isLetter(thisChar)) {
					...
					if (pos == -1) {
						...
						// let us throw one.
						//tokens.addElement(new FieldSizePair(-1, 0));
						throw new IllegalArgumentException("Close quotes!");
					}
	...
	public HybridDateFormat() {
		/*
		 * There does not appear to be a standard API for determining what the
		 * default pattern for a locale is, so use package-scope variables in
		 * DateFormatSymbols to encapsulate this.
		 */
		super();
	...
	/**
	 * Creates a date formatter using the specified pattern. The specified
	 * DateFormatSymbols will be used when formatting.
	 */
	 ...
	 * this object. This string is unlocalized.
	 * 
	...
	/**
	 * This method returns a string with the formatting pattern being used by
	 * this object. This string is localized.
	 * 
	 * @return The format string.
	 */
	...
	private String applyLocalizedPattern(String pattern, String oldChars,
			String newChars) {
	 ...
	 * 
	 * @return A <code>Date</code> representing the start of the century for two
	 *         digit years.
	 */
	 ...
	 * 
	 * @param date
	 *            A <code>Date</code> representing the start of the century for
	 *            two digit years.
	 */
	...
	public boolean equals(Object o) {
		...
		if (!(o instanceof HybridDateFormat))
			return false;

		HybridDateFormat sdf = (HybridDateFormat) o;
	...
	private int getLargestOverflowableUnit() {
		int largestUnit = -1;
		...
		while (e.hasMoreElements()) {
			...
			if (o instanceof FieldSizePair) {
				...
				case HOUR_OF_DAY0_FIELD:
					largestUnit = HOUR_OF_DAY0_FIELD;
					break;
				case HOUR_OF_DAY1_FIELD:
					largestUnit = HOUR_OF_DAY1_FIELD;
					break;
				case MINUTE_FIELD:
					if (largestUnit != HOUR_OF_DAY0_FIELD
							&& largestUnit != HOUR_OF_DAY1_FIELD) {
						largestUnit = MINUTE_FIELD;
					}
					break;
	 ...
	 * to the specified StringBuffer. The input StringBuffer is returned as
	...
	public StringBuffer format(Date date, StringBuffer buffer, FieldPosition pos) {
		...
		int largestOverflowUnit = getLargestOverflowableUnit();
		...
		while (e.hasMoreElements()) {
			...
			if (o instanceof FieldSizePair) {
				...
				switch (p.field) {
				case ERA_FIELD:
					buffer.append(formatData.getEras()[calendar
							.get(Calendar.ERA)]);
					break;
				case YEAR_FIELD:
				...
				case MONTH_FIELD:
					if (p.size < 3)
						...
						buffer.append(formatData.getShortMonths()[calendar
								.get(Calendar.MONTH)]);
					else
						buffer.append(formatData.getMonths()[calendar
								.get(Calendar.MONTH)]);
					break;
				case DATE_FIELD:
					withLeadingZeros(calendar.get(Calendar.DATE), p.size,
							buffer);
					break;
				case HOUR_OF_DAY1_FIELD: // 1-24
					withLeadingZeros(
							((calendar.get(Calendar.HOUR_OF_DAY) + 23) % 24) + 1,
							p.size, buffer);
					break;
				case HOUR_OF_DAY0_FIELD: // 0-23
					// if hours (0 based) were the largest unit, overflow
					// regular unit limits
					if (largestOverflowUnit == HOUR_OF_DAY0_FIELD) {
						// get time in millis, substract all lower fields
						// and thus get left millis
						long left_millis = calendar.getTimeInMillis()
								- calendar.get(Calendar.MINUTE) * 1000 * 60
								- calendar.get(Calendar.SECOND) * 1000
								- calendar.get(Calendar.MILLISECOND);
						// convert left millis to hours
						int left_hours = (int) (((left_millis / 1000.0) / 60.0) / 60.0);
						withLeadingZeros(left_hours, p.size, buffer);
					} else {
					...
					break;
				case MINUTE_FIELD:
					// if minutes were the largest unit, overflow regular unit
					// limits
					if (largestOverflowUnit == MINUTE_FIELD) {
						// get time in millis, substract all lower fields
						// and thus get left millis
						long left_millis = calendar.getTimeInMillis()
								- calendar.get(Calendar.SECOND) * 1000
								- calendar.get(Calendar.MILLISECOND);
						// convert left millis to minutes
						int left_minutes = (int) ((left_millis / 1000.0) / 60.0);
						withLeadingZeros(left_minutes, p.size, buffer);
					} else {
						withLeadingZeros(calendar.get(Calendar.MINUTE), p.size,
								buffer);
					}
					break;
				case SECOND_FIELD:
					// if seconds were the largest unit, overflow regular unit
					// limits
					if (largestOverflowUnit == SECOND_FIELD) {
						// get time in millis, substract all lower fields
						// and thus get left millis
						long left_millis = calendar.getTimeInMillis()
								- calendar.get(Calendar.MILLISECOND);
						// convert left millis to seconds
						int left_seconds = (int) (left_millis / 1000.0);
						withLeadingZeros(left_seconds, p.size, buffer);
					} else {
						withLeadingZeros(calendar.get(Calendar.SECOND), p.size,
								buffer);
					}
					break;
				case MILLISECOND_FIELD:
					// if seconds were the largest unit, overflow regular unit
					// limits
					if (largestOverflowUnit == MILLISECOND_FIELD) {
						// get left time in millis
						int  left_millis = (int) calendar.getTimeInMillis();
						withLeadingZeros(left_millis, p.size, buffer);
					} else {
				...
				case DAY_OF_WEEK_FIELD:
					if (p.size < 4)
						buffer.append(formatData.getShortWeekdays()[calendar
								.get(Calendar.DAY_OF_WEEK)]);
					else
						buffer.append(formatData.getWeekdays()[calendar
								.get(Calendar.DAY_OF_WEEK)]);
					break;
				...
				case DAY_OF_WEEK_IN_MONTH_FIELD:
					withLeadingZeros(calendar
							.get(Calendar.DAY_OF_WEEK_IN_MONTH), p.size, buffer);
					break;
				...
				case WEEK_OF_MONTH_FIELD:
					...
					break;
				case AM_PM_FIELD:
					buffer.append(formatData.getAmPmStrings()[calendar
							.get(Calendar.AM_PM)]);
					break;
				case HOUR1_FIELD: // 1-12
					withLeadingZeros(
							((calendar.get(Calendar.HOUR) + 11) % 12) + 1,
							p.size, buffer);
					break;
				case HOUR0_FIELD: // 0-11
					withLeadingZeros(calendar.get(Calendar.HOUR), p.size,
							buffer);
					break;
				case TIMEZONE_FIELD:
					...
					String zoneID = zone.getDisplayName(isDST,
							p.size > 3 ? TimeZone.LONG : TimeZone.SHORT);
	...
	public Date parse(String dateStr, ParsePosition pos) {
		...
		for (; fmt_index < fmt_max; ++fmt_index) {
			...
			case 'E':
				...
				match = (fmt_count <= 3 ? formatData.getShortWeekdays()
			...
			case 'M':
				...
				if (fmt_count <= 2)
					...
					is_numeric = false;
					match = (fmt_count <= 3 ? formatData.getShortMonths()
